...
class X509CertP
{
  ...
  : cert_{NULL}
  {
    ...
    if(!cert_) throw std::runtime_error(ERR_error_string(ERR_get_error(), NULL));
  }
...
class EvpPublicKey
{
  ...
  : evp_pk_{NULL}
  {
    ...
    if(!evp_pk_) throw std::runtime_error(ERR_error_string(ERR_get_error(), NULL));
  }
  ...
  : evp_pk_{NULL}
  {
    ...
    if(!fp) throw std::runtime_error(strerror(errno));
    ERR_load_crypto_strings();
