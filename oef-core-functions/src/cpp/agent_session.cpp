...
void AgentSession::process_register_description(uint32_t msg_id, const fetch::oef::pb::AgentDescription &desc) 
{
  ...
  DBEUG_(logger, "AgentSession::processRegisterDescription setting description to agent {} : {}", 
  ...
  oef_search_.register_description(*description_, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processRegisterDescription failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::REGISTER_DESCRIPTION);
        } else {
          DBEUG_(logger, "::processRegisterDescription operation successful for msg {} of agent {}", msg_id, publicKey_);
...
{
  if(!description_) {
    DBEUG_(logger, "AgentSession::processUnregisterDescription agent {} hasn't registered description", publicKey_);
  ...
  oef_search_.unregister_description(*description_, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processUnregisterDescription failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::UNREGISTER_DESCRIPTION);
        } else {
          DBEUG_(logger, "::processUnregisterDescription operation successful for msg {} of agent {}", msg_id, publicKey_);
...
void AgentSession::process_register_service(uint32_t msg_id, const fetch::oef::pb::AgentDescription& desc) 
{
  ...
  DBEUG_(logger, "AgentSession::processRegisterService registering agent {} : {}", publicKey_, pbs::to_string(desc));
 
  auto self(shared_from_this()); 
  oef_search_.register_service(service_desc, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processRegisterService failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::REGISTER_SERVICE);
        } else {
          DBEUG_(logger, "::processRegisterService operation successful for msg {} of agent {}", msg_id, publicKey_);
          // TOFIX should add a status answer, even in the case of no error 
        }
      });
}
...
{
  auto service_desc = Instance(desc.description()); 
  DBEUG_(logger, "AgentSession::processUnregisterService unregistering agent {} : {}", publicKey_, pbs::to_string(desc));
  
  auto self(shared_from_this()); 
  oef_search_.unregister_service(service_desc, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processUnregisterService failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::UNREGISTER_SERVICE);
        } else {
          DBEUG_(logger, "::processUnregisterService operation successful for msg {} of agent {}", msg_id, publicKey_);
          // TOFIX should add a status answer, even in the case of no error 
        }
      });
}
...
void AgentSession::process_search_agents(uint32_t msg_id, const fetch::oef::pb::AgentSearch &search) 
{
  ...
  DBEUG_(logger, "AgentSession::processSearchAgents from agent {} : {}", publicKey_, pbs::to_string(search));
  
  auto self(shared_from_this()); 
  oef_search_.search_agents(query, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processSearchAgents failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::REGISTER_SERVICE);
        } else {
          DBEUG_(logger, "::processSearchAgents operation successful for msg {} of agent {}", msg_id, publicKey_);
...
{
  auto query = QueryModel(search.query());
  DBEUG_(logger, "AgentSession::processQuery from agent {} : {}", publicKey_, pbs::to_string(search));
  
  auto self(shared_from_this()); 
  oef_search_.search_service(query, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processQuery failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::REGISTER_SERVICE);
        } else {
          DBEUG_(logger, "::processQuery operation successful for msg {} of agent {}", msg_id, publicKey_);
          fetch::oef::pb::Server_AgentMessage answer;
          answer.set_answer_id(msg_id);
          auto answer_agents = answer.mutable_agents();
          for(auto &a : response.agents) {
...
{
  auto query = QueryModel(search.query());
  DBEUG_(logger, "AgentSession::processQueryWide from agent {} : {}", publicKey_, pbs::to_string(search));
  
  auto self(shared_from_this()); 
  oef_search_.search_service_wide(query, publicKey_, msg_id,
      [this, self, msg_id](std::error_code ec, OefSearchResponse response) {
        if (ec) {
          DBEUG_(logger, "::processQueryWide failed operation for msg {} of agent {}", msg_id, publicKey_);
          send_error(msg_id, fetch::oef::pb::Server_AgentMessage_OEFError::REGISTER_SERVICE);
        } else {
          DBEUG_(logger, "::processQueryWide operation successful for msg {} of agent {}", msg_id, publicKey_);
          fetch::oef::pb::Server_AgentMessage answer;
          answer.set_answer_id(msg_id);
...
void AgentSession::process_message(uint32_t msg_id, fetch::oef::pb::Agent_Message *msg) 
{
  ...
  DBEUG_(logger, "AgentSession::process_message from agent {} : {}", publicKey_, pbs::to_string(*msg));
  ...
  if(session) {
    ...
    DBEUG_(logger, "AgentSession::process_message to agent {} : {}", msg->destination(), pbs::to_string(message));
